...
class TestFeatureFlagParameters:
    ...
    def test_parameter_flag_basic(self, admin_api_client):
    ...
    def test_flag_state_change(self, admin_api_client):
        ...
        with override_settings(FLAGS={'TRANSITION_FLAG': [{'condition': 'boolean', 'value': True}]}):
            cache.clear()  # Clear cache to ensure new settings are picked up
            response = admin_api_client.get(url)
...
class TestFeatureFlagViews:
    ...
    def test_flag_based_view_response(self, admin_api_client, ping_url):
    ...
    def test_flag_based_view_disabled(self, admin_api_client, ping_url):
    ...
    def test_feature_dependency_chain(self, admin_api_client, ping_url):
