...
@mock.patch("aap_gateway_api.views.api.v1.jwt_key.get_jwt_rsa_key", return_value="")
def test_jwt_key_bad_key(
    get_jwt_rsa_key, logger, unauthenticated_api_client, shut_up_logging
):
...
def test_jwt_key_bad_public_key(logger, unauthenticated_api_client, shut_up_logging):
    ...
    with mock.patch(
        "aap_gateway_api.views.api.v1.jwt_key.serialization.load_pem_private_key",
