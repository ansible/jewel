...
def test_logged_basic_auth(
    logger, unauthenticated_api_client, organization, admin_user
):
    client = unauthenticated_api_client
    client.credentials(
        HTTP_AUTHORIZATION="Basic "
        + base64.b64encode("admin:password".encode("utf-8")).decode("utf-8")
    )
...
def test_logged_basic_auth_disabled(
    unauthenticated_api_client, organization, admin_user, settings, set_preference
):
...
@mock.patch(
    "rest_framework.views.APIView.authentication_classes", [LoggedBasicAuthentication]
)
def test_logged_basic_auth_invalid(unauthenticated_api_client, organization):
    client = unauthenticated_api_client
    client.credentials(
        HTTP_AUTHORIZATION="Basic "
        + base64.b64encode("admin:wrongPassw0rd".encode("utf-8")).decode("utf-8")
    )
    url = reverse("organization-list")
    response = client.get(url)
...
def test_logged_basic_auth_invalid_disabled(
    unauthenticated_api_client, organization, set_preference
):
    set_preference("proxy", "gateway_basic_auth_enabled", False)
    client = unauthenticated_api_client
    client.credentials(
