...
class TestLoadCustomEnvvars:
    ...
    def test_fallback_cache_is_default(self, dynaconf_settings):
        """DABCacheWithFallback should be the default cache backend (works on all topologies)."""
        assert dynaconf_settings.CACHES["default"]["BACKEND"] == "ansible_base.lib.cache.fallback_cache.DABCacheWithFallback"

    def test_redis_local_cache_entry_exists(self, dynaconf_settings):
        """The sidecar Redis (Unix socket) entry should exist under the 'redis_local' key."""
        assert dynaconf_settings.CACHES["redis_local"]["BACKEND"] == "ansible_base.lib.cache.redis_cache.DABRedisCache"
        assert dynaconf_settings.CACHES["redis_local"]["LOCATION"] == "unix:///var/run/redis/redis.sock?db=4"
