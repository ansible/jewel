...
class HTTPPort(CommonModel):
    ...
    is_api_port = models.BooleanField(
        default=False, help_text="If true, this port will be used to serve the AAP service APIs. Only one port can be the API port."
    )
...
class Route(NamedCommonModel):
    ...
    service_path = models.CharField(max_length=255, blank=False, help_text="URL path on the AAP Service cluster to route traffic to.")
    gateway_path = models.CharField(max_length=255, blank=False, help_text="Path on the AAP Gateway to listen to traffic on.")
