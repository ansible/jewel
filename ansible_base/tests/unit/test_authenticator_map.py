...
from ansible_base.authentication import common
...
def test_create_claims_single_map_acl(
    ...
    res = common.create_claims(authenticator, "username", attrs, groups)
...
def test_create_claims_revoke(
    ...
    with mock.patch(f"ansible_base.authentication.common.{process_function}", return_value=None):
...
def test_process_groups(trigger_condition, groups, has_access):
    ...
    """
    res = common.process_groups(trigger_condition, groups, authenticator_id=1337)
...
def test_has_access_with_join(current_access, new_access, condition, expected):
    ...
    and None.
    """
    res = common.has_access_with_join(current_access, new_access, condition)
...
def test_process_user_attributes(trigger_condition, attributes, expected):
    res = common.process_user_attributes(trigger_condition, attributes, authenticator_id=1337)
    assert res is expected
