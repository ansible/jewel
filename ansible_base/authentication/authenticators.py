...
logger = logging.getLogger('ansible_base.authenticator.authenticators')
...
def get_authenticator_plugins() -> list:
    ...
    parent_class = __import__(class_prefix, globals(), locals(), ['authenticators'], 0)
    plugins = []
    for path in parent_class.__path__:
        for file in glob(join(path, "*.py")):
            file_name = basename(file)
            if isfile(file) and not file_name == '__init__.py':
...
def get_authenticator_class(authenticator_type: str):
    ...
    try:
        ...
        raise ImportError(f"The specified authenticator type {authenticator_type} could not be loaded")
