...
class AAPServiceCluster(AAPObject):
    ...
    def set_new_fields(self):
        ...
        if (outlier_detection_enabled := self.params.get('outlier_detection_enabled')) is not None:
            self.new_fields["outlier_detection_enabled"] = outlier_detection_enabled

        if (outlier_detection_consecutive_5xx := self.params.get('outlier_detection_consecutive_5xx')) is not None:
            self.new_fields["outlier_detection_consecutive_5xx"] = outlier_detection_consecutive_5xx

        if (outlier_detection_interval_seconds := self.params.get('outlier_detection_interval_seconds')) is not None:
            self.new_fields["outlier_detection_interval_seconds"] = outlier_detection_interval_seconds

        if (outlier_detection_base_ejection_time_seconds := self.params.get('outlier_detection_base_ejection_time_seconds')) is not None:
            self.new_fields["outlier_detection_base_ejection_time_seconds"] = outlier_detection_base_ejection_time_seconds

        if (outlier_detection_max_ejection_percent := self.params.get('outlier_detection_max_ejection_percent')) is not None:
            self.new_fields["outlier_detection_max_ejection_percent"] = outlier_detection_max_ejection_percent

        if (health_checks_enabled := self.params.get('health_checks_enabled')) is not None:
            self.new_fields["health_checks_enabled"] = health_checks_enabled

        if (health_check_timeout_seconds := self.params.get('health_check_timeout_seconds')) is not None:
            self.new_fields["health_check_timeout_seconds"] = health_check_timeout_seconds

        if (health_check_interval_seconds := self.params.get('health_check_interval_seconds')) is not None:
            self.new_fields["health_check_interval_seconds"] = health_check_interval_seconds

        if (health_check_unhealthy_threshold := self.params.get('health_check_unhealthy_threshold')) is not None:
            self.new_fields["health_check_unhealthy_threshold"] = health_check_unhealthy_threshold

        if (health_check_healthy_threshold := self.params.get('health_check_healthy_threshold')) is not None:
