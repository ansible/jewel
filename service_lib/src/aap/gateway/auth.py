...
class GatewayCommonAuth:
    ...
    def parse_jwt_token(self, request):
        ...
        if created:
            logger.warn(f"New user {user.username} created from Gateway")
        else:
            logger.info(f"User {user.username} authenticated from Gateway")
    ...
    def get_decryption_key_from_url(self, url, timeout, validate_certs):
        ...
        try:
            ...
            self.log_and_raise(f"Failed to get JWT decryption key from Gateway server: ({e.__class__.__name__}) {e}")
    ...
    def validate_token(self, token, decryption_key):
        ...
        try:
            ...
            self.log_and_raise("JWT from Gateway server has expired")
            ...
            self.log_and_raise(f"Failed to decrypt JWT from Gateway server: {e}")
        except Exception as e:
