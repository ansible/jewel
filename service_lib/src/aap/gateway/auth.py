...
logger = logging.getLogger("aap.gateway.authentication")
...
class GatewayCommonAuth:
    ...
    def parse_jwt_token(self, request):
        ...
        decryption_key = self.get_decryption_key(
            jwt_key_setting,
            validate_certs=getattr(settings, "AAP_GATEWAY_VALIDATE_CERT", True),
            timeout=getattr(settings, "AAP_GATEWAY_URL_TIMEOUT", 30),
        )
        validated_body = self.validate_token(token, decryption_key)
        user_model = get_user_model()
        user, created = user_model.objects.update_or_create(username=validated_body["sub"])
        if created:
            logger.warn(f"New user {user.username} created from AAP Gateway")
        else:
            logger.info(f"User {user.username} authenticated from AAP Gateway")
    ...
    def get_decryption_key_from_url(self, url, timeout, validate_certs):
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_and_raise(f"Failed to get 200 response from gateway: {response.status_code}")
            ...
            self.log_and_raise(f"Failed to get JWT decryption key from AAP Gateway server: ({e.__class__.__name__}) {e}")
    ...
    def validate_token(self, token, decryption_key):
        ...
        try:
            ...
            self.log_and_raise("JWT from AAP Gateway server has expired")
            ...
            self.log_and_raise(f"Failed to decrypt JWT from AAP Gateway server: {e}")
        except Exception as e:
