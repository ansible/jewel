...
import pytest
import requests
from aap.gateway.auth import GatewayCommonAuth, JWTAuthentication, default_mapped_user_fields
...
class TestGatewayCommonAuth:
    def test_init(self):
        ...
        my_auth = GatewayCommonAuth(new_user_fields)
        assert my_auth.mapped_user_fields == new_user_fields

    def test_parse_jwt_no_header(self, caplog, mocked_http):
        with caplog.at_level(logging.INFO):
            my_auth = GatewayCommonAuth()
            my_auth.parse_jwt_token(mocked_http.mocked_parse_jwt_token_get_request('without_headers'))
            assert "X-AAP-GW-TOKEN header not set for JWT authentication" in caplog.text

    def test_parse_jwt_token(self):
        pass
        # TODO: add an actual test here, "Needs Settings"

    def test_log_exception(self):
        common_auth = GatewayCommonAuth()
    ...
    def test_get_decryption_key_file_read(self, tmp_path_factory, test_encryption_public_key):
        common_auth = GatewayCommonAuth()
        temp_dir = tmp_path_factory.mktemp("aap.gateway.auth")
    ...
    def test_get_decryption_key_url_random_exception(self):
        common_auth = GatewayCommonAuth()
        url = "http://dne.cuz.junk.redhat.com"
        with pytest.raises(AuthenticationFailed, match=r"Failed to get JWT decryption key from AAP Gateway server: \(RequestException\).*"):
            common_auth.get_decryption_key(url)
    ...
    def test_get_decryption_key_url_bad_status_codes(self, status_code, mocked_http):
        with mock.patch('requests.get') as requests_get:
            ...
            common_auth = GatewayCommonAuth()
            with pytest.raises(AuthenticationFailed, match=f"Failed to get 200 response from gateway: {status_code}"):
    ...
    def test_map_user_fields(self, user_fields, token, should_save, caplog):
        common_auth = GatewayCommonAuth()
    ...
    def test_validate_token_missing_default_items(self, remove, jwt_token, test_encryption_public_key):
        ...
        with pytest.raises(
            AuthenticationFailed,
            match=f'Failed to decrypt JWT from AAP Gateway server: Token is missing the "{remove}" claim',
        ):
    ...
    def test_validate_token_expired_token(self, jwt_token, test_encryption_public_key):
        ...
        # Test the function
        common_auth = GatewayCommonAuth()
        with pytest.raises(AuthenticationFailed, match="JWT from AAP Gateway server has expired"):
            common_auth.validate_token(jwt_token.encrypt_token(), test_encryption_public_key)
...
class TestJWTAuthentication:
    def test_authenticate(self):
        pass
        # TODO: add an actual test here "Requires login from django"

    def test_process_user_data(self):
        with mock.patch("aap.gateway.auth.GatewayCommonAuth.map_user_fields") as mock_inspect:
            jwt_auth = JWTAuthentication()
            jwt_auth.process_user_data("a", "b")
            mock_inspect.assert_called_with("a", "b")

    def test_process_permissions(self, caplog):
        with caplog.at_level(logging.INFO):
            jwt_auth = JWTAuthentication()
            jwt_auth.process_permissions(None, None)
