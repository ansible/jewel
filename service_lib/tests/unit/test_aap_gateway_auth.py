...
class TestGatewayCommonAuth:
    ...
    def test_get_decryption_key_url_random_exception(self):
        common_auth = GatewayCommonAuth()
        url = "http://dne.cuz.junk.redhat.com"
        with pytest.raises(AuthenticationFailed, match=r"Failed to get JWT decryption key from Gateway server: \(RequestException\).*"):
            common_auth.get_decryption_key(url)
    ...
    def test_validate_token_missing_default_items(self, remove, jwt_token, test_encryption_public_key):
        ...
        with pytest.raises(
            AuthenticationFailed,
            match=f'Failed to decrypt JWT from Gateway server: Token is missing the "{remove}" claim',
        ):
    ...
    def test_validate_token_expired_token(self, jwt_token, test_encryption_public_key):
        ...
        # Test the function
        common_auth = GatewayCommonAuth()
        with pytest.raises(AuthenticationFailed, match="JWT from Gateway server has expired"):
            common_auth.validate_token(jwt_token.encrypt_token(), test_encryption_public_key)
